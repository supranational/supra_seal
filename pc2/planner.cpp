...
struct work_item_t {
  ...
  work_item_t(node_id_t _idx, bool _is_leaf) {
    idx = _idx;
    dependencies_ready = 0;
    is_leaf = _is_leaf;
    buf = nullptr;
  ...
  size_t leaf_num() {
    return idx.node() & ((1 << TREE_ARITY_LG) - 1);
  }
...
class scheduler_t {
public:
  typedef std::vector<work_item_t<buffer_t>> work_stack_t;
  typedef std::map<uint64_t, work_item_t<buffer_t>> work_map_t;
  typedef std::function<void(work_item_t<buffer_t>&)> hash_cb_t;
...
public:
  ...
  void reset() {
    ...
    for (size_t i = 0; i < initial_nodes; i++) {
      ...
      // not the input layer
      stack.emplace_back(node_id_t(1, initial_nodes - i - 1), true);
    }
  }

  // Returns true if there is more work to be done
  bool next(hash_cb_t hash_cb, work_item_t<buffer_t>* work) {
    if (stack.size() == 0) {
      return false;
    }

    // Pop the element
    work_item_t<buffer_t> w = stack.back();
    stack.pop_back();
    ...
    node_id_t next_layer_id(w.idx.layer() + 1, w.idx.node() / arity);

    // Record the result
    if (wip.find(next_layer_id) == wip.end()) {
      wip.emplace(next_layer_id, work_item_t<buffer_t>(next_layer_id, false));
    }
    work_item_t<buffer_t>& dependant_work = wip.at(next_layer_id);
  ...
  bool is_done() {
    ...
    work_item_t<buffer_t>& work = wip.begin()->second;
