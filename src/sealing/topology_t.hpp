...
public:
  ...
  struct sector_config_t {
    size_t hashers_per_core;
    size_t sectors;
    vector<coordinator_t> coordinators;
...
public:
  ...
  size_t                       hashers_per_core;
  set<string>                  nvme_addrs;
  map<size_t, sector_config_t> sector_configs;
    ...
    try {
      ...
      for (int i = 0; i < topology_pc1_topos.getLength(); i++) {
        ...
        for (int j = 0; j < coordinators_cfg.getLength(); j++) {
          ...
          cout << "  coord " << core << " hashers " << hashers << endl;
          sector_config.coordinators.push_back(coordinator_t(hashers_per_core, core, hashers));
        }
        sector_configs.insert(pair((size_t)sector_config.sectors, sector_config));
      }
