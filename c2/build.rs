...
fn groth16_cuda() {
    let curve_feature = "FEATURE_BLS12_381";
...
fn apply_blst_flags(nvcc: &mut cc::Build) {
    ...
    nvcc.flag("-Xcompiler").flag("-mno-avx"); // avoid costly transitions
    nvcc.flag("-Xcompiler").flag("-fno-builtin");
    nvcc.flag("-Xcompiler")
        .flag("-Wno-unused-command-line-argument");

    if !cfg!(debug_assertions) {
        nvcc.flag("-Xcompiler").flag("-O2");
    }
}
